classdef EllTubeBasic<gras.ellapx.smartdb.rels.EllTubeTouchCurveBasic
    ...
    methods (Access=protected)
        ...
        function checkTouchCurveVsQNormArray(self,tubeRel,curveRel,...
            ...
            function checkNorm(QArray,aMat,xTouchCurveMat,...
                ...
                if ~isOk
                    throwwarn('wrongInput:touchLineValueFunc',...
            ...
            function minEig=checkIntWithinExt(QExtArray,QIntArray,calcPrec)
                ...
                if minEig+calcPrec<0
                    throwerror('wrongInput:internalWithinExternal',...
                        ...
                        'tolerance %.17g, actual tolerance is %.17g'],...
                        calcPrec,abs(minEig)));
                end
    ...
    methods (Access=protected)
        function [isPos, reportStr] = isEqualScalarAdjustedInternal(...
                self,ellTubeObj,varargin)
            % ISEQUALSCALARADJUSTEDINTERNAL - compares current relation
            %   object with other relation object and
            %           returns true if they are equal, otherwise it returns false
            %
            %
            % Usage: isEq=isEqualScalarAdjustedInternal(self,otherObj)
            %
            % Input:
            %   regular:
            ...
            if ~isempty(notExistFieldList)
                throwerror('wrongInput:notConsistentTubes',...
                    'other tube doesn''t contain the following fields %s',...
                    modgen.cell.cellstr2expression(notExistFieldList));
            end
            %
            reportStr = '';
            %
            PROP_NAME_LIST={'maxTolerance','maxRelativeTolerance',...
            ...
            elseif isFirstEmpty||isSecondEmpty
                ...
                if areTimeBoundsCompared
                    if areBeginTimesDifferent
                        isPos = false;
                        reportStr=[reportStr, ...
                            sprintf('Ending times differ by %.17g. ',...
                            abs(firstTimeVec(end)-secondTimeVec(end)))];
                    elseif areEndTimesDifferent
                        isPos = false;
                        reportStr=[reportStr, ...
                            sprintf('Beginning times differ by %.17g. ',...
